    ...
    <AuthenticatedLayout>
      ...
      <div className="relative min-h-screen">
        ...
        {showAddMember && isAdmin && unassigned && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" style={{ backgroundColor: 'rgba(61, 58, 54, 0.4)' }} onClick={() => setShowAddMember(false)}>
            <div className="w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl overflow-hidden max-h-[80vh] flex flex-col" style={{ backgroundColor: colors.surface }} onClick={(e) => e.stopPropagation()}>
              <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: `1px solid rgba(61, 58, 54, 0.06)` }}>
                ...
                <button onClick={() => setShowAddMember(false)} style={{ color: colors.text.secondary }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M18 6L6 18M6 6l12 12" /></svg></button>
              </div>
              <div className="flex-1 overflow-y-auto p-4">
                {unassigned.length > 0 ? (
                  <div className="space-y-2">
                    {unassigned.map((m: { _id: Id<"members">; name: string; contact: string | null; gender: string | null }) => (
                      <button key={m._id} onClick={() => handleAddMember(m._id)} className="w-full p-3 rounded-xl flex items-center gap-3 text-left" style={{ backgroundColor: colors.bg }}>
