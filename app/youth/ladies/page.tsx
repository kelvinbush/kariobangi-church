...
const colors = {
  ...
  accent: { rose: '#c49a9a', roseLight: '#e8d4d4', sage: '#9db88c', sageLight: '#c5d4be', terracotta: '#c49a84', terracottaLight: '#e8d8cc' }
};
    ...
    <AuthenticatedLayout>
      ...
      <div className="relative min-h-screen">
        <header className="sticky top-0 z-30 px-4 h-14 flex items-center justify-between" style={{ backgroundColor: colors.bg, borderBottom: `1px solid rgba(61, 58, 54, 0.06)` }}>
          <span className="text-sm tracking-wide" style={{ color: colors.text.secondary }}>Ladies Youth</span>
          <div className="flex items-center gap-3">
            <Link href="/youth/men" className="text-xs px-3 py-1.5 rounded-full transition-colors" style={{ backgroundColor: colors.surface, color: colors.text.secondary }}>Men</Link>
            <SignedIn><UserButton /></SignedIn>
          </div>
        </header>
        <main className="max-w-2xl mx-auto px-5 py-8 pb-24">
          <div className="rounded-2xl p-5 mb-6" style={{ backgroundColor: colors.accent.rose }}>
                    ...
                    {trends.map((day) => (
                      <div key={day.date} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ backgroundColor: colors.surfaceHover }}>
                          <div className="h-full rounded-full transition-all" style={{ width: `${day.rate}%`, backgroundColor: colors.accent.rose }} />
                        </div>
