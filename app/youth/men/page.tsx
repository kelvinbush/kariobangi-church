...
export default function MenYouthPage() {
  ...
  const roster = useQuery(api.attendance.youthRoster, isAuthenticated ? { gender: "male", date: todayIso } : "skip");
  const trends = useQuery(api.attendance.youthSundayTrends, isAuthenticated ? { gender: "male", weeks: 6 } : "skip");
    ...
    <AuthenticatedLayout>
      ...
      <div className="relative min-h-screen">
        ...
        <header 
          ...
          style={{ backgroundColor: colors.bg, borderBottom: `1px solid rgba(61, 58, 54, 0.06)` }}
        >
        ...
        <main className="max-w-2xl mx-auto px-5 py-8 pb-24">
          {/* Stats */}
          <div className="rounded-2xl p-5 mb-6" style={{ backgroundColor: colors.accent.blue }}>
            <div className="flex items-center gap-6">
              ...
              <div>
                <div className="text-2xl font-light mb-1 text-white">{stats.presentToday}</div>
                <div className="text-xs text-white/60">Present today</div>
              </div>
              <div className="w-px h-10 bg-white/20" />
              <div>
                <div className="text-2xl font-light mb-1 text-white">{stats.rate}%</div>
          ...
          <div className="flex gap-2 mb-6">
            {[
              ...
              <button
                ...
                style={{
                  backgroundColor: viewMode === tab.id ? colors.accent.blueLight : colors.surface,
                  color: viewMode === tab.id ? colors.accent.blue : colors.text.secondary,
                    ...
                    <div key={member.memberId} className="p-4 rounded-xl" style={{ backgroundColor: colors.surface }}>
                      <div className="flex items-center gap-3">
                        <div 
                          ...
                          style={{ backgroundColor: member.presentToday ? colors.accent.sage : colors.text.muted }}
                        />
                        <div className="flex-1 min-w-0">
                          <div className="text-sm" style={{ color: colors.text.primary }}>{member.name}</div>
                          {member.department && (
                            <div className="text-xs" style={{ color: colors.text.muted }}>{member.department}</div>
                          )}
                        </div>
                      </div>
                    </div>
              ...
              {trends && trends.length > 0 && (
                <div>
                  <div className="text-xs mb-3" style={{ color: colors.text.muted }}>
                    Sunday Trends (6 weeks)
                  </div>
                  <div className="space-y-3">
                    {trends.map((day) => (
                      <div key={day.date} className="flex items-center gap-3">
                        <div className="w-16 text-xs shrink-0" style={{ color: colors.text.secondary }}>
                        ...
                        <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ backgroundColor: colors.surfaceHover }}>
                          <div
                            ...
                            style={{ width: `${day.rate}%`, backgroundColor: colors.accent.blue }}
                          />
                        </div>
                        <div className="w-12 text-right text-xs" style={{ color: colors.text.secondary }}>
                ...
                <div className="space-y-2">
                  {roster?.map((member) => (
                    <MemberHistoryRow
                      ...
                      onToggle={() => setExpandedMemberId(expandedMemberId === member.memberId ? null : member.memberId)}
                    />
                  ))}
                </div>
              </div>
    ...
    <div className="rounded-xl overflow-hidden" style={{ backgroundColor: colors.surface }}>
      ...
      {isExpanded && history && (
        <div className="px-4 pb-3">
          {history.length === 0 ? (
            <p className="text-xs py-2" style={{ color: colors.text.muted }}>No records</p>
          ) : (
            <div className="flex flex-wrap gap-1.5">
              ...
              {history.length > 14 && (
                <span className="px-2 py-0.5 rounded text-[10px]" style={{ backgroundColor: colors.surfaceHover, color: colors.text.muted }}>
                  +{history.length - 14}
                </span>
              )}
            </div>
