...
export type DirectoryPerson = {
  ...
  /** Consecutive service Sundays missed, counting back from the latest one. */
...
export function buildDirectoryReportHtml(opts: {
  ...
  groupNoun?: string;
  /** Adds a "Missed" column — used by the absentee follow-up report. */
  showMissed?: boolean;
}): string {
  const {
    sections,
            ...
            <tr style="background-color: ${idx % 2 === 0 ? "#ffffff" : "#fbfbfa"};">
              ...
              <td style="padding: 6px 8px; color: #525252; white-space: nowrap;">${
                ...
                showMissed
                  ? `
              <td style="padding: 6px 8px; text-align: center; font-weight: 700; color: ${
                (p.missedSundayStreak ?? 0) >= 4 ? "#b45309" : "#141414"
              };">${p.missedSundayStreak ?? 0}</td>`
                  : ""
              }
            </tr>`
        )
          ...
          <table class="data-table">
            <thead>
              <tr>
                ...
                <th>Last Seen</th>
                ${showMissed ? `<th style="text-align: center;">Missed</th>` : ""}
              </tr>
            </thead>
            <tbody>${rows}</tbody>
          </table>
      ...
      <body>
        ...
        <div class="footer">
          Report generated by the Kariobangi Church Attendance System on ${escapeHtml(
            ...
            showMissed
              ? ` "Missed" counts the most recent service Sundays missed in a row, counting back from the last Sunday the church met.`
              : ""
          }
        </div>
